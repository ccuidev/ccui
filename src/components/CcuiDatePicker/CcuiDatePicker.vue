<template>
  <ccui-input
    class="ccui-date-editor"
    :class="'ccui-date-editor--' + type"
  ...
  <div
    class="ccui-date-editor ccui-range-editor ccui-input__inner"
    :class="[
      'ccui-date-editor--' + type,
      pickerSize ? `ccui-range-editor--${pickerSize}` : '',
  ...
  >
    <input
      ...
      @focus="handleFocus"
      class="ccui-range-input"
    />
    <slot name="range-separator">
      <span class="ccui-range-separator">{{ rangeSeparator }}</span>
    </slot>
    <input
      autocomplete="off"
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="ccui-input__icon ccui-range__close-icon"
    >
    </i>
    <i :class="['ccui-input__icon', 'ccui-range__icon', triggerClass]"></i>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    clearIcon: {
      type: String,
      default: 'ccui-icon-circle-close'
    },
    name: {
      default: '',
      validator
  ...
  watch: {
    pickerVisible(val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch('CcuiFormItem', 'el.form.blur')
        }
        this.$emit('blur', this)
        this.blur()
      }
    ...
    value(val, oldVal) {
      if (
        ...
        this.dispatch('CcuiFormItem', 'el.form.change', val)
      }
    }
  },
  ...
  computed: {
    ...
    triggerClass() {
      return (
        this.prefixIcon ||
        (this.type.indexOf('time') !== -1 ? 'ccui-icon-time' : 'ccui-icon-date')
      )
    },
    ...
    handleKeydown(event) {
      ...
      if (keyCode === 9) {
        if (!this.ranged) {
          ...
          setTimeout(() => {
            if (this.refInput.indexOf(document.activeCcuiement) === -1) {
              this.pickerVisible = false
              this.blur()
              event.stopPropagation()
            }
