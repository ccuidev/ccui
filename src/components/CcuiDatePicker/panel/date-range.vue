<template>
  <transition name="ccui-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="ccui-picker-panel ccui-date-range-picker ccui-popper"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="ccui-picker-panel__body-wrapper">
        <slot name="sidebar" class="ccui-picker-panel__sidebar"></slot>
        <div class="ccui-picker-panel__sidebar" v-if="shortcuts">
          <button
            type="button"
            class="ccui-picker-panel__shortcut"
            v-for="(shortcut, key) in shortcuts"
            :key="key"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="ccui-picker-panel__body">
          <div class="ccui-date-range-picker__time-header" v-if="showTime">
            <span class="ccui-date-range-picker__editors-wrap">
              <span class="ccui-date-range-picker__time-picker-wrap">
                <ccui-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('el.datepicker.startDate')"
                  class="ccui-date-range-picker__editor"
                  :value="minVisibleDate"
                  @input="val => handleDateInput(val, 'min')"
                  @change="val => handleDateChange(val, 'min')" />
              </span>
              <span class="ccui-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                <ccui-input
                  size="small"
                  class="ccui-date-range-picker__editor"
                  :disabled="rangeState.selecting"
            ...
            <span class="ccui-icon-arrow-right"></span>
            <span class="ccui-date-range-picker__editors-wrap is-right">
              <span class="ccui-date-range-picker__time-picker-wrap">
                <ccui-input
                  size="small"
                  class="ccui-date-range-picker__editor"
              ...
              <span class="ccui-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                <ccui-input
                  size="small"
                  class="ccui-date-range-picker__editor"
                  :disabled="rangeState.selecting"
          ...
          <div class="ccui-picker-panel__content ccui-date-range-picker__content is-left">
            <div class="ccui-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                class="ccui-picker-panel__icon-btn ccui-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="leftPrevMonth"
                class="ccui-picker-panel__icon-btn ccui-icon-arrow-left"></button>
              <button
                type="button"
                @click="leftNextYear"
                v-if="unlinkPanels"
                ...
                class="ccui-picker-panel__icon-btn ccui-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="leftNextMonth"
                v-if="unlinkPanels"
                ...
                class="ccui-picker-panel__icon-btn ccui-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="ccui-picker-panel__content ccui-date-range-picker__content is-right">
            <div class="ccui-date-range-picker__header">
              <button
                type="button"
                @click="rightPrevYear"
      ...
      <div class="ccui-picker-panel__footer" v-if="showTime">
        <ccui-button
          size="mini"
          type="text"
          class="ccui-picker-panel__link-btn"
          @click="handleClear">
...
<script type="text/babel">
  ...
  import CcuiInput from '../../CcuiInput';
  import CcuiButton from 'ccui/packages/button';
  ...
  export default {
    ...
    methods: {
      ...
      handleRangePick(val, close = true) {
        ...
        // workaround for https://github.com/CcuiemeFE/element/issues/7539, should remove this block when we don't have to care about Chromium 55 - 57
        setTimeout(() => {
          this.maxDate = maxDate;
          this.minDate = minDate;
        }, 10);
    ...
    components: { TimePicker, DateTable, CcuiInput, CcuiButton }
  };
</script>
